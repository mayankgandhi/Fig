...
class RegenerationRateLimiter {
    ...
    static let shared = RegenerationRateLimiter()
    ...
    func canRegenerate(ticker: Ticker, force: Bool = false) -> Bool {
    ...
    func recordRegeneration(for ticker: Ticker) {
    ...
    func timeUntilNextAllowedRegeneration(for ticker: Ticker) -> TimeInterval {
        return queue.sync {
            guard let lastTime = lastRegenerationTimes[ticker.id] else {
    ...
    func clearHistory(for tickerID: UUID) {
        queue.async(flags: .barrier) {
    ...
    func debugStatus(for ticker: Ticker) -> String {
