...
struct AlarmDetailHeader: View {
    ...
    var body: some View {
        HStack(spacing: DesignKit.sm) {
            ...
            VStack(alignment: .leading, spacing: DesignKit.xxs) {
                ...
                Text(statusLabel)
                    ...
                    .padding(.horizontal, DesignKit.xs)
                    .padding(.vertical, 2)
        ...
        .padding(DesignKit.sm)
        .background(DesignKit.surface(for: colorScheme).opacity(0.5))
    ...
    private var statusLabel: String {
        if tickerService.getActiveAlarm(tickerID: alarm.id) != nil {
            return "Active"
        }
    ...
    private var statusColor: Color {
        if tickerService.getActiveAlarm(tickerID: alarm.id) != nil {
            return DesignKit.scheduled
        }
        return alarm.isEnabled ? DesignKit.scheduled : DesignKit.disabled
    }
}
