...
final class AddTickerViewModel {
    ...
    /// This is used when editing a ticker that was fetched from a fresh context
    func updatePrefillTemplate(_ template: Ticker?) {
        self.prefillTemplate = template
    ...
    func saveTicker() async {
        ...
        do {
            let scheduleTypeString = schedule.displaySummary
            let hasCountdown = countdown != nil
            let isRecurring = !schedule.displaySummary.contains("One-time")

            if isEditMode, let existingTicker = prefillTemplate {
                // Ensure the ticker is still in our modelContext
                // This prevents crashes from context mismatches or deallocated objects
                guard let tickerInContext = modelContext.model(for: existingTicker.persistentModelID) as? Ticker else {
                    errorMessage = "Unable to save: Ticker no longer exists"
                    showingError = true
                    return
                }

                tickerInContext.label = labelViewModel.labelText.isEmpty ? "Alarm" : labelViewModel.labelText
                tickerInContext.schedule = schedule
                tickerInContext.countdown = countdown
                tickerInContext.presentation = presentation
                tickerInContext.soundName = soundPickerViewModel.selectedSound?.fileName
                tickerInContext.tickerData = tickerData

                try await tickerService.updateAlarm(tickerInContext, context: modelContext)

                // Track alarm edited
                AnalyticsEvents.alarmEdited(
                    alarmId: tickerInContext.id.uuidString,
                    changes: ["schedule", "label", "countdown", "sound", "icon"]
                ).track()
                ...
                let ticker = Ticker(
                    ...
                    soundName: soundPickerViewModel.selectedSound?.fileName,
                    tickerData: tickerData
                )

                try await tickerService.scheduleAlarm(from: ticker, context: modelContext)

                // Track alarm created successfully
            ...
            // Track alarm creation failed
            AnalyticsEvents.alarmCreationFailed(
                error: error.localizedDescription,
                scheduleType: schedule.displaySummary
            ).track()
